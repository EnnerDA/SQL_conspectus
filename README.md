...
Соединение при котором получается декартово произведение двух таблиц. То есть первый элемент первой таблицы стыкуется с первым элементом второй таблицы, затем со вторым элементом второй таблицы и так до послежнейго, затем берется второй элемент первой таблицы и стыкуется с первым элементом второй таблицы далее со вторым элементом второй таблицы и т.д. до последнего элемента первой таблицы.

Для перекрестного соединения таблиц используют оператор ` CROSS JOIN`.

Например для получения таблицы с 365 строками дат можно сипользовать:

[Пример КроссПересечения](https://github.com/EnnerDA/SQL_conspectus/blob/main/day_of_2023.sql)

### Естественное пересечение.
Этот тип соединения делает предположение о необходимых условиях соединения, полагаясь на идентичные имена столбцов в таблицах. Например 
```mysql
 FROM account a NATURAL JOIN customer c;	
``` 
В обоих таблицах есть столбец cust_id и БД сделает объединение `INNER JOIN` или `LEFT OUTER JOIN` с условием `USING(cust_id)` тут они идентичны.
Но если нет одинакого названных столбцов, то случится перекрестное соединение. 

Вывод: ради экономии одной строчки кода об условии объединения запоминать этот метод не вижу смысла.
...
* Конструкция `CASE` с перебором вариантов 
```mysql
CASE /* значение */
    WHEN /*сравнение значения с некоторой величеной*/ THEN /*выражение для возвращения*/
    WHEN /*сравнение значения с другой величеной*/ THEN /*выражение для возвращения*/
    ELSE /*выражение для возвращения, если ни один WHEN не выполнился*/
END
```
* Конструкция `CASE` простая
```mysql
CASE /*
    WHEN /*условие*/ THEN /*выражение для возвращения*/
    WHEN /*другое условие*/ THEN /*выражение для возвращения*/
    ELSE /*выражение для возвращения, если ни один WHEN не выполнился*/
END
``` 
Просто частный случай `CASE`с перебором. Дублирующий синтаксис - отстой.

Примерб нужно подстчитать количество счетов у каждого клиента и если их больше 2 то пишем 3+.
```mysql
SELECT c.cust_id,
CASE count(*)
    WHEN 0 THEN 'None'
...
GROUP BY cust_id;
```
[Упражнения Главы 11](https://github.com/EnnerDA/SQL_conspectus/blob/main/%D0%A3%D0%BF%D1%80%D0%B0%D0%B6%D0%BD%D0%B5%D0%BD%D0%B8%D1%8F%203/exercises_11.sql)

## Глава 12. Транзакции.

Транзакция (transaction) – механизм группировки нескольких SQL-выражений, позволяющий успешно выполниться всем или ни одному из них.

То есть для выполнения например денежного перевода получится следующий набор 
1. Начать транзакцию
2. Набор SQL выражнеий проверка наличия денег на счету проверка номеров счетов проверка и пр.
3. Если все выражения выполнены формируется команда ***commit*** (фиксирование) и транзакция завершается.
3. Либо если успешного завершения не случилось формируется команда ***rollback*** (отакат) и отменяются все измменения внесенные в ходе текущей транзакции.

В MySQL действует режим автоматической фиксации. То есть после нажатия Enter все команды фиксируются.

Но можно перейти в режим явного вызова начала транзакции командой `START TRANSACTION`.

Для отключения режима автоматической фиксации в рамках целого сеанса можно использовать 
```mysql
SET AUTOCOMMIT = 0 
```
Для успешного завершения транзакции необходимо ввести команду 
```mysql
COMMIT
```
Для отмены всех изменений, сделанных с момента начала транзакции, необходимо выполнить команду
```mysql
ROLLBACK
```
Также возможны следующие сценарии завершения транзакции:
* Выключение сервера и т.п.  автоматически выполнит `ROLLBACK`
...
* Взаимоблокировка (*deadlock*)	если сервер решил, что виновна в этом данная транзакция будет выполнен `ROLLBACK`.

В рамках одной транзакции можно делать точки сохранения (*savepoint*) и использовать их для отката вместо отмены всей транзакции. Всем точкам сэйвпоинтам присваиваются имена.
```mysql
...
```
Для просмотра механизма хранения вводим
```mysql
SHOW TABLE STATUS LIKE 'transaction' \G
```
Пример использования транзакции и сэйвпоинта
```mysql
START TRANSACTION; 	
UPDATE product 	
